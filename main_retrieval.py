...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer,
    ...
    for step, batch in enumerate(train_dataloader, start=1):
        ...
        wandb.log({
            "loss": float(reduced_l), "discrimination_loss": float(reduced_discrimination_loss), "generation_loss": float(reduced_generation_loss),
        })
...
def eval_epoch(args, model, test_dataloader, device, diffusion, type=""):
    ...
    wandb.log({
        f"{type} tvr R1": new_tv_metrics['R1'], f"{type} tvr R5": new_tv_metrics['R5'], f"{type} tvr R10": new_tv_metrics['R10'],
        f"{type} tvr R50": new_tv_metrics['R50'], f"{type} tvr MR": new_tv_metrics['MR'], f"{type} tvr MeanR": new_tv_metrics['MeanR'],
        f"{type} vtr R1": new_vt_metrics['R1'], f"{type} vtr R5": new_vt_metrics['R5'], f"{type} vtr R10": new_vt_metrics['R10'],
        f"{type} vtr R50": new_vt_metrics['R50'], f"{type} vtr MR": new_vt_metrics['MR'], f"{type} vtr MeanR": new_vt_metrics['MeanR'],
               })
...
def main():
    ...
    wandb.init(project=f"{args.output_dir.split('/')[-1]}",
               config=args)
